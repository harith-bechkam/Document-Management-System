...
function Reports({ toggle, formId }) {
    ...
    const addTab = async () => {
        ...
        const newTitle = `Tab ${newTabIndex}`;
    ...
    const getReports = async () => {
        ...
        if (responseList.exists) {
            ...
            //     title: elem.stepName,
            //     report: {
            //         data: elem.report?.data || fallbackReport,
            //         rows: [],
            //         cols: []
            //     }
            // }));
    ...
    const handlePivotTableChange = async (newReport, stepId) => {
        ...
        // dispatch(updateLoaderFlag({ loader: true, text: "Saving..." }));
        const savePivotRespo = await API.savePivotReports(newReport, formId, stepId, currentTab.title);
        // dispatch(updateLoaderFlag({ loader: false, text: "" }));

        // if (!savePivotRespo.status) return toast.error(`Unable to save Pivot Report`);

        // setTabs(prevTabs =>
        //     prevTabs.map(tab =>
        //         tab.id === stepId ? { ...tab, report: newReport } : tab
        //     )
        // );
