...
const Copy = ({ toggle, toggleCreate, copySelected, multiselected }) => {
  ...
  async function copySelectedFile(bool = false) {
    ...
    if (multiselected) {
      dispatch(updateLoaderFlag({ loader: true, text: "Copying" }));
      let multicopyRespo;
      debugger
      var detect=0
      if ((multiselected.files.length > 0)||(multiselected.shortcutFiles.length > 0)) {
        detect=0
        multicopyRespo = await API.filesCopy(multiselected.files, checked, multiselected.shortcutFiles);
        if (!multicopyRespo.status) {
          dispatch(updateLoaderFlag({ loader: false, text: "" }));
          return toast.error('error occured while copying files'.replace(/\b\w/g, char => char.toUpperCase()))
        }
      }
       if ((multiselected.forms.length > 0)||(multiselected.shortcutForms.length > 0)) {
        detect=1
        multicopyRespo = await API.formsCopy(multiselected.forms, checked, multiselected.shortcutFiles);
        if (!multicopyRespo.status) {
          dispatch(updateLoaderFlag({ loader: false, text: "" }));
          return toast.error('error occured while copying forms'.replace(/\b\w/g, char => char.toUpperCase()))
        }
      }
      dispatch(updateLoaderFlag({ loader: false, text: "" }))
      ...
      if(copySelected.isShortcut){
        detect=2
        copyRespo = await API.filesCopy([], checked, [copySelected._id]);
      }else{
        detect=0
        copyRespo = await API.filesCopy([copySelected._id], checked, []);
      }
      if (!copyRespo) {
        dispatch(updateLoaderFlag({ loader: false, text: "" }));
        return toast.error('error occured during copying file'.replace(/\b\w/g, char => char.toUpperCase()))
      }
      dispatch(updateLoaderFlag({ loader: false, text: "" }));
