...
const CopyShortcut = ({ toggle, copySelected, multiselected }) => {
  ...
  async function copySelectedFile(bool = false) {
    ...
    if (multiselected) {
      ...
      let multicopyRespo;
      if (multiselected.files.length > 0) {
        multicopyRespo = await API.filesCopy(multiselected.files, checked);
      ...
      } if (multiselected.forms.length > 0) {
        multicopyRespo = await API.formsCopy(multiselected.forms, checked);
        if (!multicopyRespo.status) {
          dispatch(updateLoaderFlag({ loader: false, text: "" }));
      ...
      dispatch(updateLoaderFlag({ loader: true, text: "Copying" }));
      const copyRespo = await API.filesCopy([copySelected._id], checked);
      if (!copyRespo) {
        dispatch(updateLoaderFlag({ loader: false, text: "" }));
