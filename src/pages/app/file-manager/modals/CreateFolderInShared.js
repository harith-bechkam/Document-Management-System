...
import { addNewFile, updateLoaderFlag, updateMoveFlag } from "../../../../redux/folderSlice";
...
const CreateFolderInShared = ({ toggle, metadataMode, setMetadataMode, handleSelectedValue }) => {
  ...
  async function handleSubmit() {
    ...
    const parentData = await API.getDetails(shareMatched[1])

    let folderResponse;
    dispatch(updateLoaderFlag({ loader: true, text: "Creating Folder" }));
    folderResponse = await API.createFolder(folderName, parentData["data"].data.sectionId, parentData["data"].data._id, defaultMetaData, custmetafield, metadataMode)
    ...
    <React.Fragment>
      ...
      <ModalBody className="overflow-auto">
        ...
        <div className="nk-upload-form mb-0">
          <CreateFolderForm folderName={folderName} setFolderName={setFolderName}
            folderCreationError={folderCreationError}
            setFolderCreationError={setFolderCreationError}
            folderCreationErr={folderCreationErr}
            setFolderCreationErr={setFolderCreationErr} handleNext={handleNext}
          />
...
export default CreateFolderInShared;
