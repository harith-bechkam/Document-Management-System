...
const MyStepOnwardsHistory = () => {
    ...
    const fetchData = async (
        ...
        let workflowResponse = await API.myStepOnwardsHistory(currentPage, itemPerPage, onSearchText, sortOrder)
    ...
    const [columns, setColumns] = useState([
        {
            ...
            name: "workflow_mystep_onward_document",
            sort: true,
            show: true,
            sortOrder: 0,
            clickable:true
        },
        {
            ...
            name: "workflow_mystep_onward_owner",
            sort: true,
            show: true,
            sortOrder: 0
        },
        // {
        //     label: "Requested At",//workflow_start_time
        //     name: "workflow_mystep_onward_workflow_start_time",
        //     sort: false,
        //     show: false,
        //     sortOrder: 0
        // },
        {
            ...
            name: "workflow_mystep_onward_current_step_name",
            sort: true,
            show: true,
            sortOrder: 0
        },
        {
            ...
            name: "workflow_mystep_onward_due_date",
            sort: false,
            show: true,
            sortOrder: 0
        },
        {
            ...
            name: "workflow_mystep_onward_updatedAt",
            sort: true,
            show: true,
            sortOrder: 0
        },
        // {
        //     label: "Final Status",
        //     name: "workflow_mystep_onward_isWorkflowCompletedStatus",
        //     sort: true,
        //     show: true,
        //     sortOrder: 0
        // },
        {
            ...
            name: "workflow_mystep_onward_progress",
            sort: false,
            show: true,
            sortOrder: 0
        },
        // {
        //     label: "Comments",
        //     name: "workflow_mystep_onward_comments",
        //     sort: false,
        //     show: true,
        //     sortOrder: 0
        // },
        {
            label: "Actions",
            name: "workflow_mystep_onward_actions",
            sort: false,
            show: true,
            sortOrder: 0
        }
    ...
    const sortData = (keycolumnName, sort) => {
        ...
        const mappings = {
            workflow_mystep_onward_document: 'docName',//1st
            workflow_mystep_onward_owner: 'workflow_start_time',//2
            workflow_mystep_onward_current_step_name: 'current_step.name',//3
            // workflow_mystep_onward_due_date: 'current_step.startDate',
            workflow_mystep_onward_updatedAt: 'updatedAt',//5
            workflow_mystep_onward_isWorkflowCompletedStatus: 'WorkflowEndTime'//6
        }
    ...
    const filterAndSliceSteps = async (data, stepName) => {
        const index = data.findIndex(step => step.stepName == stepName)
        return index != -1 ? data.slice(index) : []
    }

    const filterAndSliceComSteps = async (data, stepId) => {

        const index = data.findIndex(step => step.stepId == stepId)
        return index != -1 ? data.slice(index) : []
    }

    const filterAndSliceBackupSteps = async (data, stepId) => {
        const index = data.findIndex(step => step.stageId == stepId)
        return index != -1 ? data.slice(index) : []
    }

    const historyView = async (item, startcommentsFromthisStep) => {
        ...
        let stepslicedata = await filterAndSliceSteps(data['stepInfoView'], startcommentsFromthisStep['stepName'])
        let comslicedata = await filterAndSliceComSteps(data['commentView'], startcommentsFromthisStep['stepId'])
        let combackupslicedata = await filterAndSliceBackupSteps(data['BackUps'], startcommentsFromthisStep['stepId'])
        ...
        setModalData({
            ...
            history: stepslicedata,
            comments: comslicedata,
            backup: data['type'] == 'File' ? combackupslicedata : []
        })
        ...
        <FileManagerLayout>
            <FilesBody>
                    ...
                    <BlockBetween>
                        <BlockHeadContent>
                            ...
                            <BlockDes className="text-soft">
                                <p>You have total {data['totalCount']} Approval{data['totalCount'] != 1 && 's'} History</p>
                            </BlockDes>
                        </BlockHeadContent>
                        <BlockHeadContent>
                ...
                <RegularTable
                    ...
                    privilege={{}}
                    onEditClick={{}}
                    onEyeClick={(item, startcommentsFromthisStep) => historyView(item, startcommentsFromthisStep)}
...
export default MyStepOnwardsHistory
