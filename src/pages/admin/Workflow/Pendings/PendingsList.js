...
const PendingsList = () => {
    ...
    const [columns, setColumns] = useState([
        ...
        // {
        //     label: "Final Status",
        //     name: "workflow_isWorkflowCompletedStatus",
        //     sort: true,
        //     show: false,
        //     sortOrder: 0,
    ...
    const fetchData = async (
        ...
        let workflowResponse = await API.getWorkflowPendingsList(currentPage, itemPerPage, onSearchText, sortOrder)
    ...
    const sortData = (keycolumnName, sort) => {
        ...
        //keycolumnName - dbcol
        const mappings = {
            workflow_document: "fileName", //1st col
            workflow_owner: "workflow_start_time", //2nd col
            workflow_stepName: "stepName",//3
            workflow_updatedAt: "updatedAt"//5
        }
    ...
    const historyView = async (item) => {
        ...
        setworkflowHistoryId(item['historyId'])
        settype(item['type'])

        setLoader(true)
        dispatch(updateLoaderFlag({ loader: true, text: 'Fetching Activities' }))
        let activitityRedposen = await API.getWorkflowActivities(item['historyId'], item['type'])
                    ...
                    <BlockBetween>
                        <BlockHeadContent>
                            ...
                            <BlockDes className="text-soft">
                                <p>You have total {data['totalCount']} Approval{data['totalCount'] != 1 && 's'}</p>
                            </BlockDes>
                        </BlockHeadContent>
                        <BlockHeadContent>
