...
export const BlockBetween = ({ className, size, isDetailsPage, ...props }) => {

  return (
    <div className={`${size ? `nk-block-between-${size}` : !isDetailsPage && "nk-block-between"} ${className ? className : ""}`}>
      {props.children}
    </div>
  );
};
